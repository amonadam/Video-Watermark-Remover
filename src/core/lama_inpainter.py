        ...
        try:
            ...
            if self.use_cv2_fallback:
                try:
                    ...
                    # 先使用TELEA算法进行初步修复
                    inpaint_telea = cv2.inpaint(image_bgr, mask_bin, adaptive_radius, cv2.INPAINT_TELEA)
                    ...
                    if np.sum(edge_dilated) > 0:
                        inpaint_ns = cv2.inpaint(image_bgr, mask_bin, adaptive_radius, cv2.INPAINT_NS)
